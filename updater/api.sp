...
Action:Fwd_OnPluginChecking(Handle:plugin)
{
	...
	new Function:hFunction = GetFunctionByName(plugin, "Updater_OnPluginChecking");
	
	if (hFunction != INVALID_FUNCTION && AddToForward(g_OnPluginChecking, plugin, hFunction))
	{
...
{
	new Action:result = Plugin_Continue;
	new Function:hFunction = GetFunctionByName(plugin, "Updater_OnPluginDownloading");
	
	if (hFunction != INVALID_FUNCTION && AddToForward(g_OnPluginDownloading, plugin, hFunction))
	{
...
{
	new Function:hFunction = GetFunctionByName(plugin, "Updater_OnPluginUpdating");
	
	if (hFunction != INVALID_FUNCTION && AddToForward(g_OnPluginUpdating, plugin, hFunction))
	{
...
{
	new Function:hFunction = GetFunctionByName(plugin, "Updater_OnPluginUpdated");
	
	if (hFunction != INVALID_FUNCTION && AddToForward(g_OnPluginUpdated, plugin, hFunction))
	{
